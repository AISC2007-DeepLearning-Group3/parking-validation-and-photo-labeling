...
dnn_model = load_model('dnn_model.h5')  # Keras DNN model
ada_model = joblib.load('adaboost_model.pkl')  # AdaBoost model
xgb_model = joblib.load('xgboost_model.pkl')  # XGBoost model
ensemble_model = joblib.load('ensemble_model.pkl')  # Ensemble model combining Ada, XGBoost, RandomForest

cnn_model = load_model('cnn_model.h5')  # CNN model for image classification
...
X_train = joblib.load('X_train.pkl')  # Ensure correct path
...
def preprocess_image(img):
    ...
    img_array = np.expand_dims(img_array, axis=0)  # Expanding dimensions to match model input
...
with tab2:
    ...
    if uploaded_image is not None:
        ...
        explanation = explainer.explain_instance(
            np.array(image.resize((256, 256))) / 255.0,  # Resizing to match the input size and normalizing
            get_predictions,
            top_labels=5,
            hide_color=0,
            num_samples=1000
